...
def Generation_Bulletin(page, Donner):
    ...
    def get_matiere_info(matiere_nom, etablissement):
        ...
        try:
            ...
            if result:
                ...
                elif "litt" in genre: categorie = "Littéraire"
    ...
    def generate_bulletin_html(student, classe, notes, etablissement, annee, appreciation, moyenne_gen, rang,
        ...
        # Construire l'information de contact avec BP optionnel
        ...
        context = {'etablissement': etablissement, 'contact_etablissement': contact_info,
            'devise_etablissement': devise_etablissement, 'logo_base64': logo_base64, 'annee_scolaire': annee,
            'periode': periode, 'classe': classe, 'effectif': effectif, 'nom': student[0], 'prenom': student[1],
            'matricule': student[2], 'date_naissance': date_naissance, 'sexe': student[4],
            'matieres_litteraires': notes_litt, 'matieres_scientifiques': notes_sci, 'matieres_facultatives': notes_fac,
            ...
            'moyenne_fac': moyenne_fac, 'total_coef': int(total_coef), 'total_points': f"{total_points:.2f}",
            'moyenne_generale': f"{moyenne_gen:.2f}", 'rang': rang, 'type_periode_lower': type_periode.lower(),
            'stats': stats, 'moyennes_precedentes': len(moyennes_prec) > 0, 'moyennes_precedentes_text': moyennes_prec_text,
            ...
            'appreciation': appreciation, 'date_actuelle': datetime.now().strftime("%d %B %Y"),
    ...
    def select_periode_and_classe():
        ...
        def valider_periode(e):
            if not all([type_periode.value, periode_dropdown.value, annee_field.value, devise_field.value]):
                Dialog.error_toast("Champs obligatoires manquants")
                return
            Dialog.close_dialog(periode_dialog)
            show_classes_selection(type_periode.value, periode_dropdown.value, annee_field.value, devise_field.value, selected_logo["path"], bp_field.value)
        ...
        periode_dialog = Dialog.custom_dialog(title="📅 Configuration", content=ft.Column([
            ...
            ft.Text("Configurez les paramètres", size=14, text_align=ft.TextAlign.CENTER), ft.Divider(),
            ft.Text("Période scolaire", size=12, weight=ft.FontWeight.BOLD), type_periode, periode_dropdown, annee_field, ft.Divider(),
            ft.Text("Personnalisation", size=12, weight=ft.FontWeight.BOLD), devise_field, bp_field,
        ...
        ], width=450, height=600, spacing=10, horizontal_alignment=ft.CrossAxisAlignment.CENTER, scroll=ft.ScrollMode.AUTO),
            actions=[ft.TextButton("Annuler", icon=ft.Icons.CLOSE, on_click=lambda e: Dialog.close_dialog(periode_dialog)),
    ...
    def show_classes_selection(type_periode, periode, annee_scolaire, devise_etablissement, logo_path, bp_info):
        ...
        def create_class_card(classe):
            ...
            return ft.Container(content=ft.Column([
                ...
                on_click=lambda e, c=classe_nom, ef=effectif: show_student_selection_with_checkboxes(
                    c, ef, type_periode, periode, annee_scolaire, devise_etablissement, logo_path, bp_info))
        ...
        classes_dialog = Dialog.custom_dialog(title=f"📚 Sélection classe - {periode}", content=ft.Column([
            ...
            ft.Divider(), ft.Text("Sélectionnez une classe", size=16, weight=ft.FontWeight.BOLD, text_align=ft.TextAlign.CENTER),
            ...
            actions=[ft.TextButton("Retour", icon=ft.Icons.ARROW_BACK,
    ...
    def show_student_selection_with_checkboxes(classe_nom, effectif, type_periode, periode, annee_scolaire, devise_etablissement, logo_path, bp_info):
        ...
        selection_dialog = Dialog.custom_dialog(title=f"📋 {classe_nom} - Sélection des élèves", content=ft.Column([
            ...
            ft.Divider(), select_all_checkbox, ft.Divider(),
            ...
            actions=[ft.TextButton("Retour", icon=ft.Icons.ARROW_BACK,
                on_click=lambda e: (Dialog.close_dialog(selection_dialog),
                    show_classes_selection(type_periode, periode, annee_scolaire, devise_etablissement, logo_path, bp_info))),
    ...
    def generate_all_bulletins_batch(classe_nom, students, etablissement, annee, periode, type_periode, effectif, devise_etablissement, logo_path, bp_info):
            ...
            for student in students:
                ...
                if notes:
                    ...
                    html_content = generate_bulletin_html(student, classe_nom, notes, etablissement, annee,
                        "Bon Travail", moyenne_gen, rang, periode, type_periode, effectif, devise_etablissement, logo_path, bp_info)
